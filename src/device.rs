...
#[derive(Clone, Copy)]
pub enum Release { DriveHigh, DriveLow, HiZ }

/// User-supplied bus-hold configuration (from CLI `--hold-*`).
pub struct HoldConfig { pub pin: u8, pub active: Level, pub release: Release }
...
impl BusAccess for HostBus {
    ...
    fn acquire(&mut self) -> Result<(), Self::Error> {
        let Some(g) = self.gpio.as_mut() else { return Ok(()) };
        g.set_config(GpioDirection::Output, GpioPull::None)?;
        match self.active { Level::Low => g.set_low(), Level::High => g.set_high() }
    }
    ...
    fn release(&mut self) -> Result<(), Self::Error> {
        ...
        match self.release {
            ...
            Release::DriveHigh => { g.set_config(GpioDirection::Output, GpioPull::None)?; g.set_high() }
            Release::DriveLow  => { g.set_config(GpioDirection::Output, GpioPull::None)?; g.set_low() }
        }
    }
}
...
pub fn connect(serial: Option<&str>, freq_hz: u32, cs_pin: u8, hold: Option<HoldConfig>)
    -> Result<Connected>
{
    if let Some(h) = &hold {
    ...
    hal.spi_set_config(freq_hz, SpiPhase::CaptureOnFirstTransition, SpiPolarity::IdleLow)
        .map_err(|e| anyhow!("spi_set_config failed: {e:?}"))?;

    let spi = hal.spi_device(cs_pin).map_err(|e| anyhow!("spi_device({cs_pin}) failed: {e:?}"))?;
    let bus = match hold {
        Some(h) => HostBus { gpio: Some(hal.gpio(h.pin)), active: h.active, release: h.release },
        None => HostBus { gpio: None, active: Level::Low, release: Release::HiZ },
    };

    Ok(Connected { _hal: hal, spi, bus })
}
